...
public:
  ...
  /// swap.
  void swap(APInt& oth) {
    std::swap(oth.BitWidth, BitWidth);
    if (isSingleWord())
      std::swap(oth.VAL, VAL);
    else
      std::swap(oth.pVal, pVal);
  }
  ...
  void toStringSigned(SmallVectorImpl<char> &Str, unsigned Radix = 10,
                      bool Lowercase = false) const {
    toString(Str, Radix, true, false, Lowercase);
  }
