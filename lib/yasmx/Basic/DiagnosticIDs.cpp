...
#include "yasmx/Basic/DiagnosticCategories.h"
  ...
  if (IsFirst) {
    for (unsigned i = 1; i != StaticDiagInfoSize; ++i) {
      assert(StaticDiagInfo[i-1].DiagID != StaticDiagInfo[i].DiagID &&
             "Diag ID conflict, the enums at the start of yasm::diag (in "
...
#include "lib/yasmx/Basic/CategoryNameTable.inc"
  { 0, 0 }
};
