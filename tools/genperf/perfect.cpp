    ...
    gencode  *final)                                  /* code for final hash */
{
  ...
  switch(form->mode)
  {
  case hashform::NORMAL_HM:
    initnorm(keys, alen, blen, smax, salt, final);
    break;
  case hashform::INLINE_HM:
    ...
    hashform *form)           /* TRUE if we should do a minimal perfect hash */
{
  ...
  ub4  highhash = ((form->perfect == hashform::MINIMAL_HP) ? nkeys : smax);
  int  trans = (form->speed == hashform::SLOW_HS ||
    ...
    hashform *form)                                       /* user directives */
{
  ...
  *tabb = new bstuff[*blen];
  ...
  {
    if (form->perfect == hashform::MINIMAL_HP)
    {
    ...
    hashform *form)                                       /* user directives */
{
   ...
   */
  if (form->perfect == hashform::NORMAL_HP)
  {
    if ((form->speed == hashform::FAST_HS) && (nkeys > *smax*0.8))
    {
      *smax = *smax * 2;
    }
  ...
  else
  {
    ...
    case 1: case 2: case 3: case 4: case 5: case 6: case 7: case 8:
      *alen = (form->perfect == hashform::NORMAL_HP) ? *smax : *smax/2;
      *blen = *smax/2;
      break;
    case 9:
    case 10:
    ...
    case 16:
    case 17:
      if (form->speed == hashform::FAST_HS)
      {
        *alen = *smax/2;
        *blen = *smax/4;
      }
    ...
    hashform *form)                                       /* user directives */
{
  ...
  *smax = (static_cast<ub4>(1)<<phash_log2(nkeys));
  ...
  maxalen = (form->perfect == hashform::MINIMAL_HP) ? *smax/2 : *smax;
